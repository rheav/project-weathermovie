...
const domLocalWeatherExpression = document.getElementById(
  "local-weather-expression"
);
...
const rangeSliderManagement = () => {
  if (queryModeRangeInput.value < 4000) {
    ...
    queryModeRangeInput.value > 3000 &&
    queryModeRangeInput.value < 7000
  ) {
...
const getMovieSuggestions = async function () {
  ...
  fetchedMovies = result.choices[0].message.content
    .split("\n")
...
const cardBuilder = async function (chosenMovie, index) {
  ...
  cardPosterImg.setAttribute(
    ...
    `${
      chosenMovie?.poster_path
        ...
        : "https://raw.githubusercontent.com/rheav/project-weathermovie/main/assets/img/placeholderPoster.jpeg"
    }`
  );
  cardPosterImg.setAttribute(
    "class",
    "object-contain	w-full h-auto max-w-[full] max-h-full transition duration-300 ease-in rounded-t-lg"
  );
  ...
  cardInfoPop.setAttribute(
    "class",
    `${
      parseFloat(chosenMovie.popularity.toPrecision(4)) < 50
        ? "text-red-400"
        : chosenMovie.popularity.toPrecision(4) <= 60
        ? "text-white"
        : "text-lime-400"
    } text-xs font-bold mr-4`
  );
  ...
  popIcon.setAttribute(
    ...
    `${
      parseFloat(chosenMovie.popularity.toPrecision(4)) < 50
        ? "text-red-400"
        : parseFloat(chosenMovie.popularity.toPrecision(4)) <= 60
        ? "text-white"
        : "text-lime-400"
  ...
  votesIcon.setAttribute(
    "class",
    "tiny material-icons mr-1 scale-75 text-amber-200"
  );

  votesIcon.innerHTML = `${
    parseFloat(chosenMovie.vote_average.toPrecision(3)) < 4
      ? "star_border"
      : chosenMovie.vote_average.toPrecision(3) <= 6
      ? "star_half"
      : "star"
  }`;
  ...
  cardInfoTitle.setAttribute(
    "class",
    "col-span-4 text-lg xl:text-xl font-bold text-white whitespace-normal mb-1 transition duration-300 group-hover:text-[rgb(159, 157, 255)]"
  );
  cardInfoTitle.innerText = chosenMovie.title;

  // Lançamento
  const cardInfoReleaseDate = document.createElement("h6");
  cardInfoReleaseDate.setAttribute(
    "class",
    "basis-full col-span-4 text-sm xl:text-normal mb-4 font-thin text-white whitespace-normal transition duration-300"
  );
  ...
  chosenMovie.genre_ids.map((moviesGenreId) => {
    ...
    matchedGenre
      ? (cardInfoGenresTag.innerText = matchedGenre.name)
      : (cardInfoGenresTag.innerText = "teste");
...
const createMovieArticle = () => {
  return new Promise((resolve) => {
    ...
    skeletonPoster.src =
      "https://raw.githubusercontent.com/rheav/project-weathermovie/main/assets/img/placeholderPoster.jpeg";
    skeletonPoster.className =
      "object-contain w-full h-auto max-w-full max-h-full transition duration-700 ease-in rounded-t-lg brightness-50 animate-pulse";
    ...
    voteNpop1.className =
      "col-span-4 h-2 bg-[rgba(120,119,198,0.3)] rounded-full w-1/4";
    ...
    skeletonTitle.className =
      "col-span-4 h-2 bg-[rgba(120,119,198,0.3)] rounded-full w-3/4 my-4 animate-pulse";

    // year skeleton
    const skeletonYear = document.createElement("div");
    skeletonYear.className =
      "col-span-4 h-2 bg-[rgba(120,119,198,0.3)] rounded-full w-1/4 animate-pulse";

    // genre badges wrapper skeleton
    const skletonGenreBadWrapper = document.createElement("div");
    skletonGenreBadWrapper.className =
      "flex flex-wrap gap-4 mt-[5.5rem] animate-pulse";
